...
int main() {
    ...
    for(unsigned r=0;r<16;++r) for(unsigned k=0;k<16;++k) { a[r*16+k]=I8(int((r*7+k*3)%16)-8); b[k*16+r]=I8(int((k*11+r*5)%127)-63); }
    ...
    for(unsigned r=0;r<16;++r) for(unsigned c=0;c<16;++c) { I32 sum=0; for(unsigned k=0;k<16;++k) sum+=I32(a[r*16+k])*I32(b[k*16+c]); ref[r*16+c]=sum; }
