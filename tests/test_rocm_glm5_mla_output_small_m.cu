...
int main(int argc, char **argv) {
    ...
    for (unsigned index = 0u; index < 11u; ++index) for (unsigned m : {2u, 4u, 6u}) {
        ...
        if (index == 0u) {
            ...
            for (uint64_t off : {offsets[index] + 1u, g.size - 2u, UINT64_MAX - 1u})
    ...
    std::printf("PASS MLA output rank=%u cases=%u compared_floats=%llu refusals=%u weights=original widths=2/4/6\n",
