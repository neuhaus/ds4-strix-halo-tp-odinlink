...
#define DS4_TP_RDMA_LEGACY_MAX_MSG 16384
#define DS4_TP_RDMA_BULK_MAX_MSG 131072
...
/* Keep unknown providers on the long-standing 16 KiB decode framing.  The
 * OdinLink stream provider has been byte-verified with messages through the
 * 128 KiB bulk size, so it can carry a complete gate vector in one SEND. */
static uint32_t tp_rdma_provider_decode_max_msg(const char *device_name) {
    if (device_name && strncmp(device_name, "odl_tb5_", 8) == 0)
        return DS4_TP_RDMA_BULK_MAX_MSG;
    return DS4_TP_RDMA_LEGACY_MAX_MSG;
}

static uint32_t tp_rdma_decode_max_msg_override(uint32_t provider_default) {
    const char *s = getenv("DS4_TP_RDMA_DECODE_MAX_MSG");
    if (!s || !s[0]) return provider_default;
    char *end = NULL;
    errno = 0;
    unsigned long v = strtoul(s, &end, 10);
    if (errno || !end || *end || v < 4096 || v > DS4_TP_RDMA_BULK_MAX_MSG) {
        fprintf(stderr,
                "ds4-tp: ignoring invalid DS4_TP_RDMA_DECODE_MAX_MSG='%s' "
                "(valid range 4096..%u)\n",
                s, DS4_TP_RDMA_BULK_MAX_MSG);
        return provider_default;
    }
    return (uint32_t)v;
}

static uint32_t tp_rdma_negotiate_decode_max_msg(uint32_t local,
                                                  uint32_t peer) {
...
uint32_t ds4_tp_test_rdma_provider_decode_max_msg(const char *device_name) {
    return tp_rdma_provider_decode_max_msg(device_name);
}
uint32_t ds4_tp_test_rdma_negotiate_decode_max_msg(uint32_t local,
                                                    uint32_t peer) {
    return tp_rdma_negotiate_decode_max_msg(local, peer);
}
#endif
#endif
...
static ds4_tp_devcopy_fn g_tp_devcopy;

void ds4_tp_set_devcopy(ds4_tp_devcopy_fn fn) { g_tp_devcopy = fn; }

/* Staging copy: device DMA when the backend registered one, else memcpy. */
static inline void tp_stage_copy(void *dst, const void *src, uint64_t bytes) {
    if (g_tp_devcopy && g_tp_devcopy(dst, src, bytes)) return;
...
static int tp_hello_validate_runtime_features(uint32_t local, uint32_t peer,
                                              char *err, size_t errlen) {
    if (ds4_tp_runtime_features_equal(local, peer)) return 1;
    tp_set_err(err, errlen,
               "tp hello: runtime feature mismatch (local=0x%08x peer=0x%08x)",
               local, peer);
    return 0;
}
...
static int tp_hello_validate_prefill_config(uint64_t local, uint64_t peer,
    ...
    if (!ds4_tp_glm5_mla_handoff_config_valid(local) ||
        !ds4_tp_glm5_mla_handoff_config_valid(peer)) {
        ...
        size_t errlen)
{
    ...
    /* DS4-TP-gfx1151 (patch 3): allow ROCm once its TP runtime exists.
...
/* DS4-TP-gfx1151 (direct=1 prefill fix): dedicated big-gate regions so the
 * TP prefill FFN all-reduce (`ds4_tp_big_gate_exchange`) can read/write the
 * slab directly instead of CPU-staging through it. MEASURED (live profiling,
 * 2026-08-04): 65% of big-gate time is a ~200 MB/s CPU memcpy because
 * `batch_routed_out`/`batch_ffn_out` are ordinary graph tensors outside the
 * registered slab, so `direct` is always 0 for prefill (see
 * BIG-GATE-BOTTLENECK.md). The ROW gate already proves GPU writes into this
 * same registered slab are correctly RDMA-readable (ds4_rocm.cu's
 * hipStreamWriteValue64 gate signal orders compute before the send in
 * tp_rdma_gate_exchange), at small (~16-30KB) scale; this extends the same
 * already-proven mechanism to prefill-sized (tens of MB) transfers. ROCm TP
 * builds enable it by default; `DS4_TP_BIG_DIRECT=0` is the kill switch.
 * Other builds remain opt-in with `DS4_TP_BIG_DIRECT=1`. It reserves
 * `DS4_TP_BIG_DIRECT_MAX_ROWS` rows (default 8192, matching the documented
 * "PRO long prompts" --prefill-chunk ceiling). Zero extra slab bytes when
 * unset. */
static uint32_t tp_big_direct_max_rows(void) {
    static int cached = -1;
    static uint32_t rows = 0;
    if (cached < 0) {
        const char *on = getenv("DS4_TP_BIG_DIRECT");
        int enabled;
        if (on && on[0]) {
            enabled = strcmp(on, "0") != 0;
        } else {
        ...
        if (enabled) {
            const char *rows_env = getenv("DS4_TP_BIG_DIRECT_MAX_ROWS");
            long v = (rows_env && rows_env[0]) ? strtol(rows_env, NULL, 10) : 8192;
            rows = v > 0 ? (uint32_t)v : 8192u;
        } else {
            rows = 0;
        }
        cached = 1;
...
uint64_t ds4_tp_slab_bytes(uint32_t n_layer, uint32_t n_embd) {
    ...
    uint64_t big_rows = (uint64_t)tp_big_direct_max_rows();
...
static int tp_rdma_load_api(ds4_tp_verbs_api *api) {
    ...
    /* DS4-TP-gfx1151 (patch 2): pick the verbs library explicitly.
     ...
     * the OdinLink shim must be opened by name. Do that only when the user set
     * DS4_TP_VERBS_LIB: automatically preferring an installed OdinLink shim can
     * mask native providers such as mlx5. Without the override, load system
     * libibverbs directly so Mellanox and other providers retain their normal
     * rdma-core path with no OdinLink interposition. */
    void *h = NULL;
    const char *tp_verbs_lib = getenv("DS4_TP_VERBS_LIB");
    if (tp_verbs_lib && tp_verbs_lib[0]) {
        /* DS4 selected OdinLink explicitly, so use its validated WC-reading
         * fast path unless the caller supplied an explicit provider setting.
         * This is never set for system libibverbs / mlx5. */
        if (strstr(tp_verbs_lib, "odl_tb5_verbs") &&
            getenv("ODL_VERBS_WC_STREAM_COPY") == NULL)
            (void)setenv("ODL_VERBS_WC_STREAM_COPY", "1", 0);
        h = dlopen(tp_verbs_lib, RTLD_NOW | RTLD_LOCAL);
        if (!h) {
            fprintf(stderr, "ds4-tp: cannot load DS4_TP_VERBS_LIB=%s: %s\n",
                    tp_verbs_lib, dlerror());
            return 0;
        }
    }
...
static int tp_rdma_register_and_exchange(ds4_tp *tp, char *err, size_t errlen) {
    ...
    mine.decode_max_msg = r->decode_max_msg;
    ...
    r->decode_max_msg = tp_rdma_negotiate_decode_max_msg(
            r->decode_max_msg, r->peer.decode_max_msg);
    if (r->decode_max_msg < 4096 ||
        r->decode_max_msg > DS4_TP_RDMA_BULK_MAX_MSG) {
        tp_set_err(err, errlen,
                   "tp rdma: invalid negotiated decode message size %u",
                   r->decode_max_msg);
        return 0;
    }
    ...
    fprintf(stderr,
            "ds4-tp: rdma gate vectors use %llu message%s "
...
static uint32_t g_tp_rdma_gate_profile_layers;
...
static void tp_rdma_gate_profile_print(void) {
    ...
    for (uint32_t i = 0; i < g_tp_rdma_gate_sample_count; i++) {
        ...
        const uint64_t gates_per_token =
            (uint64_t)DS4_TP_GATES_PER_LAYER *
            (uint64_t)g_tp_rdma_gate_profile_layers;
        const uint64_t token = gates_per_token ?
            (s->seq - 1u) / gates_per_token : 0u;
        fprintf(stderr,
                "{\"ds4_tp_rdma_gate_sample\":true,\"rank\":%d,"
                "\"seq\":%llu,\"token_ordinal\":%llu,"
                "\"layer\":%u,\"gate\":\"%s\","
                "\"post_send_us\":%.3f,\"send_cqe_us\":%.3f,"
...
static int tp_rdma_gate_profile_is_enabled(const ds4_tp *tp) {
    if (g_tp_rdma_gate_profile_enabled < 0) {
        ...
        if (g_tp_rdma_gate_profile_enabled &&
            ...
            g_tp_rdma_gate_profile_layers = tp->n_layer;
...
static int tp_rdma_big_gate_capable(const ds4_tp *tp) {
    const uint64_t stage_bytes =
        (uint64_t)DS4_TP_RDMA_BULK_SLOTS * DS4_TP_RDMA_BULK_MAX_MSG;
 ...
 * are queued, so each round can post its 4 MiB receive window before sending
 * the matching 128 KiB messages (DS4_TP_RDMA_BULK_MAX_MSG, kept separate
 * from the provider-negotiated limit used by the decode gate path - 128KiB
 * crosses OdinLink's >64KiB throughput-DMA threshold, versus 16KiB which
 * only ever hit its slower small-message latency path). Verify scratch
 * provides already-registered staging memory and is idle during normal
 * prefill. */
static int tp_rdma_big_gate_exchange(ds4_tp *tp,
     ...
     * NIC memory region. */
    /* DS4-TP-gfx1151 (diag): split the big-gate cost into staging-copy vs wire.
     * The staging memcpys are CPU reads of hipMalloc device memory (uncached on
     * this APU); if `direct` is false they dominate. Printed every 64 gates. */
    static double g_bg_copy_s, g_bg_wire_s, g_bg_bytes;
    /* Phase 0 of the OdinLink transport-tuning plan: the "wire+wait" bucket
     * above was one undifferentiated blob (WR build + post_recv + post_send +
    ...
    while (off < bytes) {
        ...
        for (uint32_t i = 0; i < chunks; i++) {
            ...
            lens[i] = (uint32_t)(left > DS4_TP_RDMA_BULK_MAX_MSG ?
                                 DS4_TP_RDMA_BULK_MAX_MSG : left);
            chunk_off[i] = direct ? round_bytes :
                (uint64_t)i * DS4_TP_RDMA_BULK_MAX_MSG;
            if (!direct) {
                const double c0 = g_bg_trace ? tp_now_sec() : 0.0;
                tp_stage_copy(stage_send + chunk_off[i],
        ...
        const double pr0 = g_bg_trace ? tp_now_sec() : 0.0;
        ...
        for (uint32_t i = 0; i < chunks; i++) {
            ...
            /* DS4-TP-gfx1151 (direct=1 EAGAIN fix, superseded): this used to
             * post exactly one send at a time because OdinLink's verbs shim
             * requeued an EAGAIN-blocked WR at the TAIL of its software SQ,
             * which could reorder or drop a WR under a full 64-WR linked
             * post. The shim now retains the SQ head on EAGAIN instead
             * (verified via its "retaining SQ head" diagnostic in the
             * deployed libodl_tb5_verbs.so), so that hazard no longer
        ...
        const double ps0 = g_bg_trace ? tp_now_sec() : 0.0;
        if (ibv_post_send(r->qp, send_wr, &bad_send) != 0) {
            fprintf(stderr, "ds4-tp: bulk rdma post_send: %s\n",
                    strerror(errno));
            return 0;
        }
        const double ps1 = g_bg_trace ? tp_now_sec() : 0.0;
        if (g_bg_trace) g_bg_postsend_s += ps1 - ps0;
        ...
        while (recv_done < chunks || send_done < sends_signaled) {
            ...
            if (n < 0) return 0;
            if (g_bg_trace) {
                if (n == 0) bg_empty_this_round++;
                else if (!bg_seen_first) {
                    bg_seen_first = true;
                    g_bg_towait_first_s += tp_now_sec() - ps1;
                }
            }
            for (int i = 0; i < n; i++) {
                if (wc[i].status != IBV_WC_SUCCESS) {
        ...
        if (g_bg_trace) {
            g_bg_empty_polls += bg_empty_this_round;
            g_bg_rounds++;
        }
        atomic_thread_fence(memory_order_acquire);
    ...
    if (g_bg_trace) {
        ...
        g_bg_copy_s += bg_copy;
        g_bg_wire_s += total - bg_copy;
        g_bg_bytes  += (double)bytes;
        if (++g_bg_gates % 16u == 0u) {
            const double tot = g_bg_copy_s + g_bg_wire_s;
            fprintf(stderr,
                    "ds4-tp: big-gate %llu gates direct=%d | staging-copy %.1f ms (%.0f%%) "
                    "| wire+wait %.1f ms (%.0f%%) | %.1f MB moved -> copy %.0f MB/s, effective %.0f MB/s\n",
                    (unsigned long long)g_bg_gates, (int)direct,
                    g_bg_copy_s * 1e3, tot > 0 ? 100.0 * g_bg_copy_s / tot : 0.0,
                    g_bg_wire_s * 1e3, tot > 0 ? 100.0 * g_bg_wire_s / tot : 0.0,
                    g_bg_bytes / 1e6,
                    g_bg_copy_s > 0 ? (g_bg_bytes * 2.0) / g_bg_copy_s / 1e6 : 0.0,
                    tot > 0 ? g_bg_bytes / tot / 1e6 : 0.0);
            /* Phase 0 breakdown of the "wire+wait" bucket above: where inside
             * post/poll does the time actually go? All numbers are PER-ROUND
             * (a round = one linked post_recv+post_send+poll cycle, up to 32
             * chunks), not per-gate, since a big gate can span multiple
             * rounds when bytes > 32*128KiB. */
            ...
            fprintf(stderr,
                    ...
                    g_bg_towait_lastsend_s * 1e6 / rounds,
                    (double)g_bg_empty_polls / rounds);
        }
    }
    return 1;
}
