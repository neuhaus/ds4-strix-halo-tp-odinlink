...
typedef enum {
    DS4_GPU_DECODE_ATTN_EVENT_START = 0,
    DS4_GPU_DECODE_ATTN_EVENT_QKV_PROJ,
    DS4_GPU_DECODE_ATTN_EVENT_QKV_NORM_ROPE,
    DS4_GPU_DECODE_ATTN_EVENT_Q_B_PROJ,
    DS4_GPU_DECODE_ATTN_EVENT_Q_NORM_ROPE,
    ...
    DS4_GPU_DECODE_ATTN_EVENT_ATTN_OUTPUT_EXPAND,
    DS4_GPU_DECODE_ATTN_EVENT_ATTN_GATE,
    DS4_GPU_DECODE_ATTN_EVENT_ATTN_OUTPUT,
    DS4_GPU_DECODE_ATTN_EVENT_ATTN_HC_POST,
    DS4_GPU_DECODE_ATTN_EVENT_FFN_HC_PRE,
    DS4_GPU_DECODE_ATTN_EVENT_FFN_NORM,
    DS4_GPU_DECODE_ATTN_EVENT_ROUTER,
    DS4_GPU_DECODE_ATTN_EVENT_SHARED_GATE_UP,
    DS4_GPU_DECODE_ATTN_EVENT_SHARED_DOWN,
    DS4_GPU_DECODE_ATTN_EVENT_ROUTED_MOE,
    DS4_GPU_DECODE_ATTN_EVENT_FFN_GATE,
    DS4_GPU_DECODE_ATTN_EVENT_FFN_HC_POST,
...
typedef enum {
    ...
    DS4_GPU_VERIFY_STAGE_EVENT_ATTN_QKV_END,
    ...
    DS4_GPU_VERIFY_STAGE_EVENT_ATTN_CORE_END,
    DS4_GPU_VERIFY_STAGE_EVENT_ATTN_OUTPUT_END,
...
typedef enum {
    DS4_GPU_Q4K_PACKED_ROW_RANGE = 1,
    DS4_GPU_Q4K_PACKED_K_RANGE = 2,
} ds4_gpu_q4k_packed_slice_kind;

/* Declare and materialize a nonlinear Q4_K routed-expert slice.  ROW_RANGE
 * retains a contiguous subset of output rows.  K_RANGE retains a block-
 * aligned byte range from every output row.  Declaration immediately blocks
 * generic linear residency for the source tensor; later compute must resolve
 * the exact packed descriptor. */
...
uint32_t ds4_gpu_q4k_wmma_runtime_features(
        int q4k_weights,
        uint32_t expert_in_dim,
        uint32_t expert_mid_dim,
        uint64_t gate_expert_bytes,
...
void ds4_gpu_tp_route_profile(const ds4_gpu_tensor *selected,
                              ...
                              uint32_t pos);
#endif
/* Diagnostic-only FFN partial-sum range profiler.  A no-op unless
 * DS4_TP_FFN_RANGE_PROFILE is exactly "1". */
void ds4_gpu_tp_ffn_range_profile(const ds4_gpu_tensor *tensor,
                                  uint64_t elements, uint32_t layer,
                                  int prefill_big);
...
int ds4_gpu_embed_token_hc_bf16_tensor(
        ds4_gpu_tensor *out_hc,
        const void       *model_map,
        uint64_t          model_size,
        uint64_t          weight_offset,
...
int ds4_gpu_embed_tokens_hc_bf16_tensor(
        ds4_gpu_tensor       *out_hc,
        const ds4_gpu_tensor *tokens,
        const void             *model_map,
        uint64_t                model_size,
...
int ds4_gpu_embed_token_hc_q8_0_tensor(
        ds4_gpu_tensor *out_hc,
        const void       *model_map,
        uint64_t          model_size,
        uint64_t          weight_offset,
...
int ds4_gpu_embed_tokens_hc_q8_0_tensor(
        ds4_gpu_tensor       *out_hc,
        const ds4_gpu_tensor *tokens,
        const void             *model_map,
        uint64_t                model_size,
 ...
 * cache contract. The causal batch-lora entry point below accepts the same
 * NoPE contract; arbitrary selected-row batch entry points remain guarded. */
